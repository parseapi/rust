...
- Breaking: `TariffDeep.measures` is now `Option<Vec<TariffMeasure>>`; handle unresolved `None` separately from a resolved empty list.
...
Email responses also expose the existing core `free` and `domain_type` fields.
