...
impl Smoke {
	...
	fn ok<T>(&mut self, name: &str, result: Result<T>, assert: impl FnOnce(&T) -> Option<String>) {
		match result {
	...
	fn err<T>(&mut self, name: &str, result: Result<T>, code: &str) {
		match result {
...
async fn main() {
	...
	s.ok("ip", parse.ip("8.8.8.8", None).await, |r| {
		(r.ip == "8.8.8.8")
			.then_some(None)
			.unwrap_or(Some("wrong ip".into()))
	});
	s.ok("ip_self", parse.ip_self(None).await, |r| {
		(!r.ip.is_empty())
			.then_some(None)
			.unwrap_or(Some("no ip".into()))
	});
	...
	s.ok(
		"bloc_countries",
		parse.bloc_countries("SCHENGEN").await,
		|r| {
			(r.countries.len() == 29)
				.then_some(None)
	...
	s.ok("country_states", parse.country_states("US").await, |r| {
		(r.states.len() >= 50)
			.then_some(None)
			.unwrap_or(Some("too few".into()))
	});
	s.ok(
		"state",
		parse
			.state("NC", parseapi::StateOptions::default().country("US"))
			.await,
		|r| {
			(r.name == "North Carolina")
				.then_some(None)
				.unwrap_or(Some("wrong".into()))
		},
	);
	s.ok(
		"state_districts",
		parse
			.state_districts(
				"NC",
				parseapi::StateDistrictsOptions::default().country("US"),
			)
			.await,
		|r| {
			(!r.districts.is_empty())
				.then_some(None)
				.unwrap_or(Some("empty".into()))
		},
	);
	s.ok("district", parse.district("37081", None).await, |r| {
		r.name
			.contains("Guilford")
			.then_some(None)
			.unwrap_or(Some("wrong district".into()))
	});
	s.ok(
		"city",
		parse
			.city("charlotte", parseapi::CityOptions::default().country("US"))
			.await,
		|r| {
			if r.name != "Charlotte" {
	...
	s.ok(
		...
		parse
			.city_search(
				"char",
				parseapi::CitySearchOptions::default()
					.country("US")
					.limit(5),
			)
			.await,
	...
	s.ok(
		...
		|r| {
			(r.distance >= 0.0)
				.then_some(None)
				.unwrap_or(Some("no distance".into()))
		},
	);
	s.ok(
		"postal",
		parse
			.postal("28202", parseapi::PostalOptions::default().country("US"))
			.await,
		|r| {
			(r.city.as_deref() == Some("Charlotte"))
				.then_some(None)
				.unwrap_or(Some("wrong city".into()))
		},
	);
	s.ok(
		"postal_nearby",
		parse
			.postal_nearby(
				"28202",
				parseapi::PostalNearbyOptions::default()
					.radius(40.0)
					.country("US"),
			)
			.await,
	...
	s.ok(
		...
		parse
			.postal_distance(
				"28202",
				"10001",
				parseapi::PostalDistanceOptions::default().country("US"),
			)
			.await,
		|r| {
	...
	s.ok("phone", parse.phone("+14155552671", None).await, |r| {
		(r.phone.as_deref() == Some("+14155552671"))
			.then_some(None)
			.unwrap_or(Some("wrong phone".into()))
	});
	// Metered core siblings: junk numbers return valid false without a metered lookup.
	s.ok(
		"carrier junk free",
		parse.carrier("555-0100", None).await,
		|r| {
			(!r.valid)
				.then_some(None)
				.unwrap_or(Some("expected invalid".into()))
	...
	s.ok(
		"caller junk free",
		parse.caller("555-0100", None).await,
		|r| {
			(!r.valid)
				.then_some(None)
				.unwrap_or(Some("expected invalid".into()))
	...
	s.ok("domain", parse.domain("gmail.com", None).await, |r| {
		(!r.available)
			.then_some(None)
			.unwrap_or(Some("gmail available?".into()))
	});
	s.ok("asn", parse.asn("AS13335").await, |r| {
		(r.asn == 13335)
			.then_some(None)
			.unwrap_or(Some("wrong ASN".into()))
	});
	s.ok("mac", parse.mac("00:1B:63:84:45:E6").await, |r| {
		(r.valid
			&& r.mac == "00:1B:63:84:45:E6"
			&& r.local == Some(false)
			&& r.multicast == Some(false))
		.then_some(None)
	...
	s.ok(
		"vin junk",
		parse.vin("1HGCM82613A004352", None).await,
		|r| {
			(!r.valid)
				.then_some(None)
				.unwrap_or(Some("expected invalid".into()))
	...
	s.ok("currency", parse.currency("USD").await, |r| {
		(r.symbol.as_deref() == Some("$"))
			.then_some(None)
			.unwrap_or(Some("wrong symbol".into()))
	});
	s.ok(
		"currency_rate",
		parse.currency_rate("USD", "EUR", None).await,
		|r| {
			(r.rate > 0.0 && r.rate < 10.0)
				.then_some(None)
				.unwrap_or(Some("bad rate".into()))
		},
	);
	...
	s.ok(
		"holiday null",
		parse.holiday_date("US", "2026-08-12").await,
		|r| {
			r.holiday
				.is_none()
				.then_some(None)
				.unwrap_or(Some("expected null".into()))
		},
	);
	s.ok("elevation", parse.elevation(35.2271, -80.8431).await, |r| {
		r.elevation
			.is_some()
			.then_some(None)
			.unwrap_or(Some("no elevation".into()))
	});
	...
	s.ok(
		...
		|r| {
			r.station
				...
				.then_some(None)
				.unwrap_or(Some("no station".into()))
		},
	);
	s.ok("emoji", parse.emoji("rocket").await, |r| {
		(r.emoji == "\u{1F680}")
			.then_some(None)
			.unwrap_or(Some("wrong emoji".into()))
	});
	s.ok(
		"emoji_search",
		parse
			.emoji_search("fire", parseapi::EmojiSearchOptions::default().limit(5))
			.await,
		|r| {
			(!r.emojis.is_empty())
				.then_some(None)
	...
	s.ok(
		...
		parse
			.point(
				36.0726,
				-79.792,
				parseapi::PointOptions::default().deep(true),
			)
			.await,
		|r| {
			r.deep
				.is_some()
				.then_some(None)
				.unwrap_or(Some("deep missing".into()))
		},
	);

	s.err(
		"honest 404",
		parse.city("notarealcityxyz", None).await,
		"not_found",
	);
	let bogus = Client::builder()
		.api_key("bogus_key_123")
		.retries(0)
		.build()
		.expect("client");
	s.err(
		"bogus key 401",
		bogus.country("US").await,
		"invalid_api_key",
	);
